...
----------------------------------------------
Compilación --> python rc4_joelescobar.py
nota: tener previamente instalado python en el ordenador
"""

import os
#Método KSA para el cifrado RC4 -> Key Scheduling Algorithm 
def KSA(key):
    i = 0
    klenght = len(key)
...
#Método PGRA para el cifrado RC4 -> Pseudo-Random Generation  Algortihm  (PGRA)
# Usa el Algoritmo KSA para generar la secuencia de clave y sirve para cifrar y descifrar
def PGRA(S, tamaño_mensaje):
    ...
    while tamaño_mensaje > 0:
        ...
        i = (i + 1) % 256 # i toma el valor de la iteracion 
        j = (j + S[i]) % 256  # sera la suma del valor inicial mas la posicion de i 
        S[i], S[j] = S[j], S[i] # se intercambian los valores
        K = S[(S[i] +  S[j]) % 256] #sumamos los valores de los bytes y le aplicamos el modulo
        key.append(K) #añadimos la nueva posicion al vector
    return key
